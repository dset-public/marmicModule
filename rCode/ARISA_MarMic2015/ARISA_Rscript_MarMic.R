...
ShortNames<-as.character(sapply(as.character(Data3$Sample.File.Name),function(x){
...
identify(NMDS_M$points[,1],NMDS_M$points[,2], labels=M[,ncol(M)]) #interactive selection of point to print label
...
table(droplevels(M[failedPCR,"G"]))
