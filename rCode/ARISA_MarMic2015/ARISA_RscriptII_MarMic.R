...
minSEQ=0
maxSEQ=10000 #adjust value to position when curves level out
plot(maxSEQ,max(nOTU),type="n",xlim=c(minSEQ,maxSEQ),ylim=c(0,max(nOTU)))
...
#legend
legend(max(Merged_nmds$points[,1]),min(Merged_nmds$points[,2])+diff(range(Merged_nmds$points[,2]))*0.5,
