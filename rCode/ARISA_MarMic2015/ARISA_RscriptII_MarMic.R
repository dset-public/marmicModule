...
par(
    xpd = NA,       # banishes the restriction to
                    # only plot within the 
                    # plotting area. Now also in the
                    # margins
    mar = c(5,5,2,10), # set margin spaces around 
                       # the plot c(bottom, left, 
                       # top, right)
    cex.axis = 0.8,    # character expansion for axes
    col.axis = "black" # axes colour
    )            
...
plot(
  c(0, max(nOTU)) ~ c(0,max(as.numeric(env$int))),
  type = "n",
  axes = F,
  ylab = "nOTU",
...
axis(
  side = 1, # set the x axis
  at = seq(0,max(as.numeric(env$int)),1),
  labels = c("t0",levels(env$int))
  )

axis(
  side = 2,
  at = seq(0, max(nOTU), 20),
  labels = T, # use the same numbers as 'at'
  las = 2
  )

points(
  nOTU[env$time == "t0"] ~ rep(0,length(nOTU[env$time == "t0"])),
...
points(
  nOTU[env$time!="t0"] ~ as.numeric(env$int)[env$time!="t0"],
  pch = env$pch[env$time!="t0"],
  col=env$col[env$time!="t0"],
  cex=1.5
  )

# check out the data itself
# data.frame(nOTU, env$int)
...
rect.hclust(Merged_clust, h = 0.6) # Group at a height
                                   # of 0.6
...
plot(
  Merged_nmds$points[,1],
  Merged_nmds$points[,2],
  type = "n",
  axes = F,
  ylab = "",
  xlab = ""
...
#legend
legend(
  max(Merged_nmds$points[,1]),
  max(Merged_nmds$points[,2]),
  legend = c(
    "Time", "t0",
    "",
    "Temperature", "0*C", "5*C", "18*C", "28*C",
    "",
    "Spirulina","control","fed"),
  col = c("white", "darkgrey",
          "white",
          "white","lightblue","darkgreen","orange","darkred",
          "white",
          "white","black","black"),
  pch = c(rep(15,11),17),
  pt.cex = 1.3
...
# checking for overlap and differences in OTUs per sample...
setdiff(
 colnames(Merged[1, which(Merged[1, ] != 0)]), 
 colnames(Merged[6, which(Merged[6, ] != 0)])
)
